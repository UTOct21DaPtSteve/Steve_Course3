...
# File: Course3Task3_Steve_0304.R
...
plot(svmPred1, oobTest$Volume, xlab= "Predicted Values", ylab="Observed Values")
...
plot(rfPred1, oobTest$Volume, xlab= "Predicted Values", ylab="Observed Values")
abline(a = 0, b = 1, lwd=2, col = "green")
...
set.seed(123)
oobSVMfit4ProdTypes <- train(Volume ~. + ProductTypePC + ProductTypeLaptop + ProductTypeNetbook + ProductTypeSmartphone, data = oobTrain, method = "svmLinear",
                   trControl=fitControl,
                   preProcess = c("center", "scale"),
                   tuneLength = 10)


svmPred4ProdTypes <- predict(oobSVMfit4ProdTypes, oobTest)

postResample(svmPred4ProdTypes, oobTest$Volume)
#       RMSE    Rsquared         MAE 
#413.1368199   0.9688884 276.0563568
...
set.seed(123)
oobRFfit4ProdTypes <- train(Volume ~. + ProductTypePC + ProductTypeLaptop + ProductTypeNetbook + ProductTypeSmartphone, data=oobTrain, method="rf", importance=T, trControl=fitControl)

rfPred4ProdTypes <- predict(oobRFfit4ProdTypes, oobTest)

postResample(rfPred4ProdTypes, oobTest$Volume)
#       RMSE    Rsquared         MAE 
#334.6574967   0.7569658 191.7667429 
...
oobGBMfit4ProdTypes <- train(Volume ~. + ProductTypePC + ProductTypeLaptop + ProductTypeNetbook + ProductTypeSmartphone, data=oobTrain, method="gbm", trControl=fitControl)

gbmPred4ProdTypes <- predict(oobGBMfit4ProdTypes, oobTest)
...
svmPred2 <- predict(oobSVMfit4ProdTypes, shortenedreadyData2)
